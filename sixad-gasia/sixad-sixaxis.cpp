...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        fprintf(stderr, "Running %s requires 'sixad'. Please run sixad instead\n", argv[0]);
        return 1;
    }
    ...
    if (settings.rumble.enabled) {
      // FIXME
      if (pthread_kill(uinput_listen_thread, SIGTERM)) {
      ...
      if (settings.rumble.old_mode) {
          // FIXME
          if (pthread_kill(rumble_listen_thread, SIGTERM)) {
